                ...
                <tr>
                  ...
                  {active.map(t => (
                    <th key={t.n} className="compare-th-tool">
                      <img src={getLogo(t.url)} alt={t.n} style={{ width: 28, height: 28, borderRadius: 6, objectFit: 'contain', background: 'var(--bg3)', padding: 2, margin: '0 auto 6px' }} onError={e => e.target.style.display = 'none'} />
              ...
              <tbody>
                {rows.map(row => (
                  <tr key={row.label}>
                    ...
                    {active.map(t => <td key={t.n} className="compare-td">{row.fn(t)}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
